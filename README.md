...
The repository contains a collection of [Ansible](https://www.ansible.com/) "playbooks" that provision and configure machines to develop, and run [Hyperledger](http://hyperledger.org).  In particular, the Ansible playbooks automate the creation of an instance of the [Hyperledger Fabric](https://github.com/hyperledger/fabric) using [Consul](https://www.consul.io/), [Docker](https://www.docker.com/), [Docker Swarm](https://docs.docker.com/swarm/overview/), and [Docker Compose](https://docs.docker.com/compose/).  

The actions of the playbooks themselves are configured through an inventory file of host computers to use, and a small collection of variables, with sensible defaults, that allow all aspects of the Fabric deployment to be specified.  For instance, an arbitrary number of instances of Fabric "Peers" can be launched across a networked collection of hosts simply by changing the value of a single variable.  Also, the exact Docker images to use, and the exact hosts to use, and their roles, are all configurable. The playbooks will take care of the details of automatically provisioning the various hosts with Consul (servers and clients), Docker, Docker Swarm (managers and nodes), and Docker Compose, as appropriate.  The playbooks will then automatically generate and install the necessary Fabric configuration files. Finally, the playbooks will take care of automatically starting the configured Fabric instance on a secure [overlay network](https://docs.docker.com/swarm/networking/).  
...
Ansible operates by using ssh and [scp](https://en.wikipedia.org/wiki/Secure_copy) to copy the code for a module (task) to the host computer.  That code is then executed, potentially with sudo privileges, and allowed to examine the state of the machine.  If there is something to do, like install software, it does so, if not, it simply exits.  In either case, a module returns a JSON object back, as text through [stdout](https://en.wikipedia.org/wiki/Standard_streams#Standard_output_.28stdout.29), over the network to Ansible running on the control machine with a summary of the changes it did, or did not, make.  
...
### playbooks
   ...
   The playbooks directory has a sub-directory called [roles](playbooks/roles/README.md). This sub-directory contains a collection of directories, one for each role.  The names of those directories are, by convention, the names of the roles.  When a playbook references a role, it lists the name of one of these directories.  For example, the role "dockerhost" included in the playbook "playbooks/dockerhosts.yml" references the directory "[playbooks/roles/dockerhost](playbooks/roles/dockerhost/README.md)" (note that the names of the playbooks and the names of roles do not usually match so directly).
...
5. Copy the user's public SSH Key to the new host.  This only populates the ".ssh/authorized\_keys" file with the id\_rsa.pub value.  It doesn't create any other files on the host.  For example, for the user "daford" on a host with ip address 10.12.34.100 issue this on the Ansible controlling machine.
...
The playbooks and the roles they reference provide the basic specification of how to provision and configure a system to run a distributed instance of Hyperledger over a collection of different hosts.  To enable this functionality, additional configuration is required to specify the exact hosts that will participate, and their roles in the network. This is accomplished by editing the fabrichosts file.  Also, the characteristics of the Hyperledger Fabric itself needs to be specified, for instance, the exact Docker images to be used for the peers, and the member service, as well as their number.  This configuration will require editing the group\_vars/all.yml file; the variable that require customization are documented in the [README.md](group_vars/README.md) in the group_vars directory.

# Using the playbooks

After the fabrichosts and group\_vars/all.yml files have been customized, the playbooks can be executed to provision, configure and launch an instance of the Hyperledger Fabric on a distributed collection of networked hosts. 
...
Next, configure the Consul servers (Note, there is also a playbook/shell script to provision and configure Consul clients consulclients.sh):

        bin/consulservers.sh

Next, provision and configure the Docker Swarm nodes:

        bin/swarmnodes.sh
         
Next, provision and configure the Docker Swarm managers:

        bin/swarmmanagers.sh

Lastly, bring up the Fabric, by default this will launch eight (8) validating peers and one member service:
...
With the Fabric now running, one can list the status of the various peers and their assignments to the  different hosts participating in the swarm:

        docker -H <ip of swarmmanager>:4000 ps
        

To use Docker Compose, login to the host listed in fabricchosts in the "fabriclaunchers" group and change directories to the ~/Fabric/configurable directory.

        cd ~/Fabric/configurable
        
The following will display information about the hosts in the Docker Swarm.  This will list the hosts that the swarm manager knows about.  This is useful to double-check that the swarm nodes are known to the swarm manager. 

        docker-compose -H <ip of swarmmanager>:4000 info
...
The Fabric can be re-launched with:

        docker-compose -H <ip of swarmmanager>:4000 up
...
To bring down the default noops fabric with four hosts, which will be launched by default if the fabric variable is not set to "configurable", log into the Fabric launcher host and issue:
...
It likely means that the host is updating the list of available software installation packages and has a lock on the process.  One of the things the playbooks do is to also update the available software installation packages to make sure that the appropriate versions are available and installed, hence the conflict.  This is more likely to be encountered shortly after a machine has booted.  The only thing to do is to try again later when the host has completed the process.